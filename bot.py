...
NPC_TICK_SECONDS = int(os.getenv("NPC_TICK_SECONDS", "45") or 45)
...
async def npc_world_tick(context: ContextTypes.DEFAULT_TYPE):
    """چند رویداد پشت‌سرهم تا دنیا شلوغ و زنده باشد."""
    ...
    try:
        ...
        if summaries:
            logger.info("NPC tick (%s): %s", len(summaries), " | ".join(summaries))
            # کانال: خلاصه فشرده
            body = "📰 <b>بولتن زنده جهان</b>\n" + "\n".join(f"• {s}" for s in summaries)
...
async def on_bot_startup(app: Application):
    ...
    if jq is None:
        logger.warning(
            "JobQueue نیست؛ requirements باید python-telegram-bot[job-queue] باشد."
        )
        return
    interval = max(20, int(NPC_TICK_SECONDS))
    jq.run_repeating(npc_world_tick, interval=interval, first=min(20, interval))
    logger.info("NPC AI ON — every %ss, multi-action ticks", interval)
